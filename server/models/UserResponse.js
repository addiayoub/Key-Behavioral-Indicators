...
const userResponseSchema = new mongoose.Schema({
  ...
  // Champs originaux de UserResponse
  responses: [{
    questionId: {
      type: Number,
      required: true
  ...
  // ============ CHAMPS AJOUTÉS D'EMPLOYEERESPONSE ============
  clientId: { 
  ...
  employeeId: { 
    type: mongoose.Schema.Types.Mixed, // Peut être ObjectId ou String
    required: false 
  },
  sessionId: { 
    type: String, 
    required: false // Pas requis pour les UserResponse originaux
  },
  employeeName: {
    type: String,
    required: false
...
userResponseSchema.index({ sessionId: 1 });
userResponseSchema.index({ clientId: 1, createdAt: -1 });
userResponseSchema.index({ employeeId: 1 });
...
userResponseSchema.methods.getResponseSummary = function() {
...
userResponseSchema.methods.getKBIScores = function() {
  return {
...
userResponseSchema.methods.getCategoryScores = function() {
  return this.categoryScores || [];
};

// Méthode pour obtenir le score total
userResponseSchema.methods.getTotalScore = function() {
  return {
    score: this.score,
...
// Méthode pour vérifier si c'est un employé ou un utilisateur anonyme
userResponseSchema.methods.isEmployeeResponse = function() {
  return !!(this.clientId || this.sessionId || this.employeeId);
};

// Méthode pour obtenir les informations de l'employé
userResponseSchema.methods.getEmployeeInfo = function() {
  return {
